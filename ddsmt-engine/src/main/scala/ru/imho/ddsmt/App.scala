...
import ru.imho.ddsmt.params.{Param, Hour}
import ru.imho.ddsmt.ds.{CheckStrategies, DirectoryDsConfig, GenericDsConfig}
...
object App {
  ...
  def main(args: Array[String]): Unit = {
    ...
    val paramFabric: Map[String, String => Iterable[Param]] = Map("Hour" -> (v => Hour.apply(v)))
    ...
    val loadLogsFromAdFox = new RuleConfig(paramDesc, adFoxFtpDS, adFoxLocalDS, new ExtCommandConfig(Iterable("/data/temp/writeToFile.sh", "wget --limit-rate=3M ftp://imho-video:OltUjLeit1@ftp.adfox.ru/$hour.eve.gz >> " + adFoxLocalDS + " # Загрузка лог-файлов из Adfox по ftp 1/час")))
